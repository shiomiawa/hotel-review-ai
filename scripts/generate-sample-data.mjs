...
const FRAGMENTS = {
  room: {
    pos: [
      { text: "部屋が広く、家族4人でもゆったり過ごせました。", stay: ["家族"] },
      "加湿空気清浄機があり、乾燥が気になりませんでした。",
      "ベッドの寝心地が良く、朝までぐっすり眠れました。",
      "アメニティが充実していて、手ぶらでも困りませんでした。",
      "掃除が行き届いていて清潔でした。",
      { text: "デスクと椅子がしっかりしていて、夜の仕事がはかどりました。", stay: ["ビジネス", "一人旅"] },
  ...
  net: {
    ...
    neg: [
      ...
      { text: "夜に仕事をしたかったのに、回線が不安定で困りました。", stay: ["ビジネス"] },
  ...
  service: {
    pos: [
      ...
      "チェックアウト後も荷物を預かっていただき、身軽に観光できました。",
  ...
  food: {
    pos: [
      "朝食の地元野菜のビュッフェが美味しく、朝から幸せな気分になりました。",
      "名物のだし巻き卵が絶品で、思わずおかわりしました。",
      "ラウンジの無料コーヒーが美味しく、観光の合間にほっとできました。",
...
const OPENERS = {
  家族: () => pick(["家族旅行で1泊しました。", "子ども2人と家族で泊まりました。"]),
  カップル: () => pick(["夫婦で1泊しました。", "記念日旅行で利用しました。"]),
  ビジネス: () => pick(["出張で1泊しました。", "仕事で利用しました。"]),
  一人旅: () => "一人旅で1泊しました。",
  友人: () => pick(["友人3人で泊まりました。", "学生時代の友人との旅行で利用しました。"]),
  連泊: () => `${2 + Math.floor(rand() * 2)}泊で利用しました。`,
};
const CLOSERS = {
  pos: ["また利用したいです。", "次はもっとゆっくり泊まりたいです。", "友人にもすすめたいと思います。", ""],
...
function negRate(axis, monthIndex) {
  switch (axis) {
    case "room":
      return 0.3;
    case "net":
      return monthIndex >= 9 ? 0.6 + 0.07 * (monthIndex - 9) : 0.08;
    case "service":
...
function generateReview(monthIndex, y, m, d) {
  ...
  const axes = Object.keys(MENTION).filter((axis) => {
    let p = MENTION[axis];
    if (axis === "net" && stayType === "ビジネス") p += 0.35;
    return chance(p);
  });
...
const LONG_COMPLAINTS = [
  {
    ...
    stay_type: "ビジネス",
    site: "楽天トラベル",
    text: "出張で1泊しました。夜にどうしても参加しなければならないオンライン会議があり、Wi-Fiが使えることを確認して予約しましたが、会議中に3回も接続が切れました。フロントに相談したところ「ルーターを再起動します」とのことでしたが改善せず、結局スマホのテザリングで乗り切り、通信量の追加料金もかかりました。予約サイトに『全館Wi-Fi完備』と書いてある以上、通信環境は最優先で見直していただきたいです。",
  },
  {
    month: "2026-03",
    rating: 2,
    stay_type: "家族",
    site: "じゃらん",
    text: "チェックイン後に部屋へ入ると、タオルが1人分足りず、ゴミ箱には前のお客さんのゴミが残っていました。フロントに伝えると「確認します」と言われたまま連絡がなく、結局自分で取りに行きました。夜にもう一度お願いしたところ、別のスタッフの方が丁寧に謝ってくださり、すぐに対応していただけましたが、最初の対応で不信感が残りました。部屋自体は広くて使いやすかっただけに、残念です。",
  },
  {
    month: "2026-07",
    rating: 1,
    stay_type: "家族",
    site: "Google",
    text: "7月に家族で1泊しましたが、部屋のエアコンがほとんど効かず、夜も室温が28度から下がりませんでした。夜9時ごろに部屋の交換をお願いしましたが満室とのことで、扇風機を貸していただくのみでした。子どもが寝苦しくて何度も起きてしまい、楽しみにしていた家族旅行が台無しになりました。設備の点検をもう少しこまめにしていただきたいです。",
  },
  {
    month: "2025-11",
    rating: 2,
  ...
  },
  {
    ...
    stay_type: "ビジネス",
    site: "楽天トラベル",
    text: "出張で2泊しました。昼間は問題ないのですが、夜8時以降になるとWi-Fiが極端に遅くなり、翌日の会議資料のアップロードに1時間近くかかりました。ラウンジのWi-Fiも同じ状況でした。部屋の設備やスタッフの対応には満足しているので、回線の増強さえしていただければ、また出張で利用したいと思っています。",
  },
  {
    month: "2026-01",
    rating: 2,
...
const TRICKY = [
  ...
  { month: "2026-08", rating: 3, stay_type: "ビジネス", text: "Wi-Fiが遅くて仕事にならず困りましたが、ラウンジのコーヒーに救われました。" },
  { month: "2026-02", rating: 3, stay_type: "ビジネス", text: "朝食は普通。部屋も普通。可もなく不可もなく。" },
  { month: "2026-07", rating: 5, stay_type: "ビジネス", text: "Wi-Fiの調子が悪いと伝えたら、スタッフさんがすぐにルーターを交換してくれました。対応が早くて助かりました。" },
  ...
  { month: "2026-09", rating: 3, stay_type: "ビジネス", text: "ラウンジではWi-Fiがサクサクで快適…と思ったら、部屋に戻ると全然つながらない。" },
...
placeHandwritten({ date: LAST_DAY, rating: 5, stay_type: "カップル", text: "大変親切な接客でした。" }, "one-liner");
for (let k = 0; k < 29; k++) {
